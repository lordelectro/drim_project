                        ...
                        @if(!empty($match))
                            @foreach($match as $ev)
                                ...
                                <tr  >
                                    ...
                                    <td data-sortindex="0" onclick="SendToBetslip('KhmabDlNEeiA1AAVXS-e4g', '{{$ev->match_hometeam_name}}', {{$ev->odd_1x}}, 'Soccer', 'Match Result (1X2)',
                                            '{{$ev->match_hometeam_name}} vs {{$ev->match_awayteam_name}}', '{{$ev->match_date}}'
                                            , '5e99192a-4d39-e811-80d4-00155d2f9ee2');"
                                    >

                                    <span>

                                        <a href="#" class="btn-sm bettingtd btn btn-success" >{{$ev->odd_1x}}</a>

                                    </span>

                                    </td>

                                    <td data-sortindex="2" onclick="SendToBetslip('KhmabTlNEeiA1AAVXS-e4g', 'Draw', {{$ev->odd_1x}}, 'Soccer', 'Match Result (1X2)',
                                            '{{$ev->match_hometeam_name}} vs {{$ev->match_awayteam_name}}',
                                            '{{$ev->match_date}}', '5e99192a-4d39-e811-80d4-00155d2f9ee2');">
                                        <span>
                                            <a href="#" class="btn-sm bettingtd btn btn-info" role="button">{{$ev->odd_1x}}</a>
                                        </span></td>

                                    <td>{{$ev->match_awayteam_name}}</td>
                                    <td  data-sortindex="1" onclick="SendToBetslip('KhmabjlNEeiA1AAVXS-e4g', '{{$ev->match_awayteam_name}}' , {{$ev->odd_12}}, 'Soccer', 'Match Result (1X2)',
                                            '{{$ev->match_hometeam_name}} vs {{$ev->match_awayteam_name}}', '{{$ev->match_date}}', '5e99192a-4d39-e811-80d4-00155d2f9ee2');" >

                                    <span>


                                    <a href="#" class="btn-sm bettingtd btn btn-success" role="button">{{$ev->odd_12}}</a>
